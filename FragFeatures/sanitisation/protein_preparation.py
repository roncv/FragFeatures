...
from openmm.app import PDBFile, Modeller, ForceField, Simulation, PME, LJPME
...
class ProteinPreparation:
	...
	def add_hydrogens(self, minimize=True, pH=7.8):
		...
		residues_to_remove = ['DMS', 'TRS', 'LIG', 'CL'] # TODO: Make into constant # +'CL'
		...
		# Debugging
		if self.verbose_l2:
			unique_residues = set([residue.name for residue in modeller.topology.residues()])
			...
			residues = [f"{resname}{resid}_{residx+1}" for resname, resid, residx in zip(resnames, resids, openmm_resids)]
			print(f"\nResidues: \n\n{residues}")
		...
		try:
			...
			if self.verbose:
				print(f"Termini: (lower_idx: {min(openmm_resids)}, higher_idx: {max(openmm_resids)})")
			if self.verbose_l2:
				print(f"\nResIdxs:\n{openmm_resids}\n")
			termini_idxs = [min(openmm_resids), max(openmm_resids)-1]
			for residx in termini_idxs:
				modeller.delete([residue for residue in modeller.topology.residues() if residue.index == residx])
			openmm_resids_cut = [residue.index for residue in modeller.topology.residues()]
			if self.verbose_l2:
				print(f"\nResIdxs Cut:\n{openmm_resids_cut}\n")
			modeller.addHydrogens(forcefield, pH=pH)
			self.termini = False
