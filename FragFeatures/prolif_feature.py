...
class PLFeature():
	...
	def get_feature_metadata(self):
		...
		if self.verbose_l2:
			print("\n-----------------------")
			print(f"ProLIF feature metadata")
			print("-----------------------\n")
			print(f"Interaction type: {self.interaction_type}")
			print("")
			...
			# TODO: Add ligand feature metadata
	...
	def extract_prot_feat_mdata_from_universe(self):
		...
		if len(self.protein_indices) > 1:
			...
			logger.warning(f"More than one protein atom associated with protein feature")
		else:
	...
	def extract_lig_feat_mdata_from_rdkit(self):
		...
		self.ligand_atom_symbols = [atom["atom_symbol"] for atom in self.ligand_atoms.values()]
		self.ligand_atom_objs = [atom["atom_obj"] for atom in self.ligand_atoms.values()]
		self.ligand_atom_charges = [atom["atom_charge"] for atom in self.ligand_atoms.values()]
		self.ligand_atom_num_explicit_hs = [atom["atom_num_explicit_hs"] for atom in self.ligand_atoms.values()]
		self.ligand_atom_smarts = [atom["atom_smarts"] for atom in self.ligand_atoms.values()]
