...
class DUckCompound:
    ...
    def get_wqb(self, feature_dir):
        ...
        with open(wqb_file, newline="") as tsvfile:
            ...
            if wqb_column_header != "WQB":
                print(
                    f"Warning: Second column header is '{wqb_column_header}' instead of 'WQB'."
                )
    ...
    def get_summaries(self):
        ...
        for feature_dir, feature_name in zip(
            self.feature_dirs, self.simulation_dirnames
        ):
    ...
    def compound_dir(self):
        ...
        if not os.path.isdir(compound_dir):
            raise FileNotFoundError(
                f"Compound directory '{compound_dir}' does not exist."
            )
    ...
    def simulation_dirnames(self):
        ...
        simulation_dirnames = [
            d
            for d in os.listdir(compound_dir)
            if os.path.isdir(os.path.join(compound_dir, d))
        ]
        # Check for the presence of a simulation directory
        simulation_dirnames = [
            d
            for d in simulation_dirnames
            if os.path.isdir(os.path.join(compound_dir, d, self.duck_sim_dirname))
        ]
        if not simulation_dirnames:
            logger.warning(
                f"No simulation directories found for compound '{self.compound_id}'."
            )
    ...
    def simulation_dirs(self):
        ...
        simulation_dirs = [
            os.path.join(compound_dir, d, self.duck_sim_dirname)
            for d in simulation_dirnames
        ]  # full paths
    ...
    def feature_dirs(self):
        ...
        feature_dirs = [
            os.path.join(compound_dir, d) for d in simulation_dirnames
        ]  # full paths
        if not feature_dirs:
            logger.warning(
                f"No simulation directories found for compound '{self.compound_id}'."
...
if __name__ == "__main__":
    ...
    df = pd.read_json(
        "/Users/nfo24278/Documents/dphil/diamond/DuCK/structures/CHIKV_Mac_simulations/Experiment/cx0270a/analysis/wqb_summary.json"
    )
