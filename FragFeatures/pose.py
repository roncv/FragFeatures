...
class Pose:
    ...
    def define_interaction_types(self, selected_interaction_types):
        ...
        if selected_interaction_types not in valid_interactions:
            raise ValueError(
                f"Invalid interaction types: {selected_interaction_types}. Valid options are: {valid_interactions}"
            )
        ...
        updated_feature_families = [
            f for f in feature_families for _ in range(interaction_types.count(f))
        ]

        # Modify the complementary features dict to include only defined interaction types
        updated_complementary_features = {
            f: complementary_features[f]
            for f in updated_feature_families
            for _ in range(interaction_types.count(f))
        }
    ...
    def calculate_fingerprint(self):
        ...
        for family in ligand_families:  # NOTE: was feature families before
            compound_features_by_family[family] = [
        ...
        for prot_feature in protein_features:
            ...
            prot_residue = protein_system.get_chain(prot_feature.res_chain).residues[
                f"n{prot_feature.res_number}"
            ]
            ...
            if isinstance(complementary_family, list):
                ...
                complementary_comp_features = compound_features_by_family[
            ...
            if (
                len(prot_feature.atoms) > 1
            ):  # protocol if more than one atom for protein feature
                ...
                if len(valid_features) >= 1:  # print warning if feature is valid
                    logger.warning(
                        f"More than one ligand atom associated with protein feature {feature_duck_name}: {prot_feature.atoms}"
                    )  # TODO: Add compound code here
            ...
            sparse_fingerprint[prot_feature.family_name_number_chain_atoms_str] = (
                len(valid_features),
                feature_duck_name,
            )  # just the number of ligand atoms + name
            sparse_fingerprint_ext[prot_feature.family_name_number_chain_atoms_str] = (
                prot_feature,
                valid_features,
                feature_duck_name,
            )  # extended: includes feature details
        ...
        dense_fingerprint_ext = {
            k: v for k, v in sparse_fingerprint_ext.items() if len(v[1]) > 0
        }
        ...
        dense_fingerprint_str = "\n".join(
            [f"{k}: {v}" for k, v in dense_fingerprint.items()]
        )
        dense_fingerprint_ext_str = "\n".join(
            [f"{k}: {v}" for k, v in dense_fingerprint_ext.items()]
        )
